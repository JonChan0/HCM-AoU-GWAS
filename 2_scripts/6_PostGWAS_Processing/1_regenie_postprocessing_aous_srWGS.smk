'''
This pipeline performs downstream processing on AoUS per-chromosome .regenie outputs from Step 2
Author: Jonathan Chan
Date: 2024-11-04
'''
chr = list(range(1,23)) #Chr 1-22

rule all:
    input:
        rsid_tsvs = expand("{regenie_filepath}{pheno}_step2_chr{chr}_{pheno}_rsID_formatted.tsv", regenie_filepath=config['regenie_filepath'], pheno=config['pheno'], chr=chr),
...
rule rsID_mapper: #This requires internet access so needs to be run locally or on login node
    input:
        input_regenie="{regenie_filepath}{pheno}_step2_chr{chr}_{pheno}.regenie"
    output:
        rsid_tsv="{regenie_filepath}{pheno}_step2_chr{chr}_{pheno}_rsID.tsv"
    params:
...
rule rsID_tsv_formatter:
    ...
    output:
        rsid_formatted_tsv="{regenie_filepath}{pheno}_step2_chr{chr}_{pheno}_rsID_formatted.tsv"
    params:
        pheno = config['pheno'],
        output_folder=config['regenie_filepath']
    resources:
...
rule perchr_merger:
    input:
        expand("{regenie_filepath}{pheno}_step2_chr{chr}_{pheno}_rsID_formatted.tsv", regenie_filepath=config['regenie_filepath'], pheno=config['pheno'], chr=chr)
    output:
